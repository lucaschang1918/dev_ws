  ...
  while (rclcpp::ok()) {
    try {
      ...
      if (header[0] == 0x5A) {
        ...
        // RCLCPP_INFO(this->get_logger(), "detect_color = %X", packet[0]);
        ...
        if (crc_ok) {
          ...
          //   (packet.game_time > 0 && packet.game_trclcppime <= 29)) {
  ...
  const auto_aim_interfaces::msg::TimeInfo::ConstSharedPtr time_info)
{
  ...
  try {
    ...
    packet.yaw = msg->yaw;
    RCLCPP_INFO(this->get_logger(), "packet.state: %d", packet.state);
...
//     // RCLCPP_WARN(get_logger(), packet.yaw);
//     packet.vx = rune->a;
//     packet.vy = rune->b;
//     packet.vz = rune->w;
//     packet.v_yaw = 0.0;
